...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    nc, act, scales, head_channel, fusion_mode = (d.get(x) for x in ("nc", "activation", "scales", "head_channel", "fusion_mode"))
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            args = [ch[f], *args]  # 插入 in_planes
            c2 = ch[f]             # 该模块不改变通道数
        elif m is Multiply:
            c2 = ch[f[0]]          # 输出通道数 = 第一个输入的通道数
        elif m is Add:
            c2 = ch[f[0]]          # 输出通道数 = 第一个输入的通道数
        elif m is Fusion:
            inc_list = [ch[x] for x in f]
            args.insert(0, inc_list) # 插入 inc_list
            # 检查 fusion 模式以确定输出通道
            fusion_mode = args[1] if len(args) > 1 else 'bifpn' # 默认为 bifpn
        ...
        elif m in {Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect,AYHead}:
