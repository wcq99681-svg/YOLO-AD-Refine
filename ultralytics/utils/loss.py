...
class EMASlideLoss:
    def __init__(self, loss_fcn, decay=0.999, tau=2000):
        ...
        # --- 修正开始 ---
        # 1. 将 decay 和 tau 保存为实例属性
        self._decay_rate = decay
        self._tau = tau
        # 2. 将 self.decay 指向一个常规的类方法
        self.decay = self._calculate_decay
        # --- 修正结束 ---
    ...
    # 3. 添加这个新的类方法
    def _calculate_decay(self, x):
        """使用存储的 decay_rate 和 tau 计算衰减值"""
        return self._decay_rate * (1 - math.exp(-x / self._tau))
    
    def __call__(self, pred, true, auto_iou=0.5):
        if self.is_train and auto_iou != -1:
            self.updates += 1
            # 这一行现在会调用 _calculate_decay 方法，可以被 pickle
...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max=16):
        ...
        # NWD
        self.nwd_loss = True
        self.iou_ratio = 0.5#0.5 # total_iou_loss = self.iou_ratio * iou_loss + (1 - self.iou_ratio) * nwd_loss
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask, mpdiou_hw=None):
        ...
        if self.use_wiseiou:
            ...
            # iou = bbox_inner_mpdiou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, mpdiou_hw=mpdiou_hw[fg_mask], ratio=0.9) #0.7
            # iou = bbox_focaler_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, PIoU2=True, d=0.0, u=0.95)
